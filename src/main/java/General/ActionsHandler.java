...
public class ActionsHandler extends ActionsHandlerEssentials {
	...
	public void processAction(ParserOutput action) {
		...
		case PRENDI:
			...
			if (firstObject != null && secondObject == null) {
				...
				if (secondObject instanceof GenericObjectContainer
					...
					lasciaOPrendiOggetto(firstObject, secondObject, protagonist, false);
				} else {
					printer.objectNotInRoomMessage();
				}
			} else {
			...
			if (firstObject != null) {
				if (currentObjects.contains(firstObject)) {
					if (eventHandler instanceof PackageEventHandler) {
						...
						if (eventHandler.isStarted() && firstObject.equals(eventHandler.getEvent().getEnigma())) {
		...
		case LASCIA:
			if (firstObject != null && secondObject == null) {
				...
				if (protagonist.isInInventory(firstObject)) {
					lasciaOPrendiOggetto(firstObject, secondObject, protagonist, true);
				} else {
					printer.objectNotInInventoryMessage();
				}
			} else {
		...
		case GUARDA:
			...
			if (firstObject != null && secondObject == null) {
				...
				if (!game.getCurrentRoom().getCharacters().isEmpty()) {
					printer.printMessage("Nella stanza ci sono:");
		...
		case PARLA:
			...
			break;
		case USA:
			if (firstObject != null && eventHandler == null && currentRoom.getName().contains("Executive")
			...
			} else if (firstObject != null && eventHandler != null) {
				if (currentObjects.contains(firstObject)) {
					if (eventHandler instanceof CoffeEventHandler && firstObject.getObjectName().equals("macchinetta")) {
						printer.printMessage(((Coffe) (firstObject)).switchOn());
						eventHandler.endEvent(protagonist);

					} else if (eventHandler instanceof PanelEventHandler && firstObject.getObjectName().equals("pannello")) {
						printer.printMessage(((Panel) (firstObject)).switchOn());
						eventHandler.endEvent(protagonist);

					} else if (eventHandler instanceof CarEventHandler && firstObject.getObjectName().equals("grimaldello")) {
						eventHandler.endEvent(protagonist);
						printer.printMessage("La serratura si e' aperta");
						currentObjects.remove(firstObject);

					} else if (firstObject.getObjectName().equals("chiavi")) {
						if (eventHandler instanceof PhysicsEventHandler && (action.getFirstAdjective().equals("fisica"))
								|| eventHandler instanceof DibDoorEventHandler
										&& (action.getFirstAdjective().equals("dib"))) {
							eventHandler.endEvent(protagonist);
	...
	private void lasciaOPrendiOggetto(GenericObject firstObject, GenericObject secondObject, Protagonist protagonist,
			boolean drop) {
		if (game.getCurrentRoom().isInRoom(secondObject)) {
			GenericObjectContainer containerObjectRoom = (GenericObjectContainer) secondObject;
			if (containerObjectRoom.isOpened()) {
				if (drop) {
					protagonist.removeObject(firstObject);
					containerObjectRoom.addToContainer(firstObject);
					printer.printObjectLeftIn(firstObject, secondObject);
				} else {
					if (containerObjectRoom.contains(firstObject)) {
						containerObjectRoom.removeFromContainer(firstObject);
						protagonist.addObject(firstObject);
			...
			GenericObjectContainer containerObjectInventory = (GenericObjectContainer) secondObject;
	...
	private void apriOChiudiOggetto(GenericObject firstObject, GenericObject secondObject, Protagonist protagonist,
		...
		if (firstObject != null && secondObject == null) {
			...
			if (firstObject instanceof GenericObjectContainer) {
				GenericObjectContainer containerObject = (GenericObjectContainer) firstObject;
				if ((game.getCurrentRoom().isInRoom(containerObject)
						|| protagonist.isInInventory(containerObject))
